...
class EmptyAction extends BaseAction{
	...
	public function index(){
		$cityName = $this->_arctype->getTypeIs(MODULE_NAME);//验证这个action是否存在
		if(empty($cityName)){	
			$cityName = strtolower(MODULE_NAME);
			if(preg_match('/^guide\-[\w-\-]+$/',$cityName)){
				...
				if(count($_guide) == 2 && $_guide[1] == "list"){
					if(preg_match('/^[\w-\-]+$/',$_guide[0])){
						$cityName = $this->city($_guide[0]);//验证这个action是否存在
		...
		if($_data['ranks'] == 2){//当是国家是，就判断一下
			$_position = $this->_arctype->getPosition(MODULE_NAME);//获得当前位置	
