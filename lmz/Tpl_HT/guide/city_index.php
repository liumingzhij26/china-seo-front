...
<div class="container">
	...
	<div class="row">
                	...
                	<div class="tab-pane active" id="hot">
						...
						<div class="posts">
                            <div class="thumbnail">
                                ...
                                <a href="__HT__/guide-{$val['id']}.html"><img src="__static__/images/travel/screenshoot.jpg" /></a>
								<else/>
								<a href="__HT__/guide-{$val['id']}.html"><img src="{$val['litpic']}" width='178px' height='115px' /></a>
								</if>
                            </div>
                            <div class="posts_cont">
                                <h3><a href="__HT__/guide-{$val['id']}.html">{$val['title']}</a></h3>
                                <div class="views">浏览<span>{$val['click']}</span></div>
                                <div><span>作者：{$val['writer']}</span><span>发表时间：{$val['pubdate']|date="Y-m-d",###}</span><span>分类：<a href="__HT__/{$val['typedir']}-guide/">{$val['typename']}</a></span></div>
        ...
        <div class="col-md-4 left-clear">
        	<div class="sidemenu">
            	<ul>
	            	<li class="sl_1"><a <if condition="($data.explain neq '')">href="__HT__/guide-{$data.explain.id}.html"</if> class="btn btn-default">{$data.obj.typename}概况<i></i></a></li>
                    <li class="sl_2"><a <if condition="($data.scene neq '')">href="__HT__/guide-{$data.scene.id}.html"</if> class="btn btn-default">{$data.obj.typename}景点<i></i></a></li>
                    <li class="sl_3"><a href="__HT__/{$data.obj.typedir}-guide-list/" class="btn btn-default">{$data.obj.typename}游记<i></i></a></li>
                    <li class="sl_4"><a href="__HT__/{$data.obj.typedir}/" class="btn btn-default">{$data.obj.typename}酒店<i></i></a></li>
            ...
            <div class="panel">
                ...
                <span class="more_right"><a href="__HT__/{$data.obj.typedir}-guide-list/">更多&gt;&gt;</a></span>
                <ul class="lists">
                	<foreach name="scriptures" item="val" key="key">
						<li><a class="lists_a" href="__HT__/guide-{$val['id']}.html"><span class="glyphicon glyphicon-play"></span>{$val['title']}</a></li>					
					</foreach> 
                </ul>
            </div>
            <div class="panel">
            	<h2>{$data.obj.typename}最新预订酒店</h2>
                <span class="more_right"><a href="__HT__/{$data.obj.typedir}/">更多&gt;&gt;</a></span>
...
<div class="container">
	<div class="panel padding dlists">
                ...
                <p>
				<foreach name="data.city_list" item="val" key="key"><span><a href="__HT__/{$val['typedir']}-guide/">{$val['typename']}</a></span></foreach></p>
				</if>
                <h4>亚洲热门国家</h4>
                <p><foreach name="data.country_list" item="val" key="key"><span><a href="__HT__/{$val['typedir']}-guide/">{$val['typename']}</a></span></foreach></p>
	</div>
</div>
<div class="container">
...
$(function(){
	...
	$('.nav').find('a').click(function(){
		...
		$.post("__HT__/{$data.obj.typedir}-guide/",{"order":guide_form.val()},function(data){
			if(data == 0){	
				return;
			}else{
				var obj = eval('(' + data + ')');
...
function getPage(order_val,page){
	...
	$.post("__HT__/{$data.obj.typedir}-guide/",{"order":order_val,"page":page},function(data){
		if(data == 0){	
			return;
		}else{
			var obj = eval('(' + data + ')');
