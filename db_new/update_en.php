	...
	foreach($_Hotel as $k=>$v){
		if($_Hotel->eq($k)->attr('name') != ""){
			...
			if(empty($_flag_1) && empty($_flag_2) && empty($_flag_3)){				
				...
				$database->query("update dede_addonarticle set en_title = '{$_data['name']}' where aid = {$_data['id']};");	
				echo "update dede_addonarticle set en_title = '{$_data['name']}' where aid = {$_data['id']};";
				echo "<hr/>";
			}
		}
